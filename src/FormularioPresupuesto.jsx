...
export default function FormularioPresupuesto({ onVolver }) {
  ...
  async function generarExcel() {
    ...
    try {
      ...
      if (ocultarSinUsar) {
        CAPITULOS.forEach((cap, ci) => {
          let algunaUsada = false;
          cap.items.forEach((it, ii) => {
            const fila = cap.items_start + ii;
            const v = valores[`${ci}-${ii}`];
            const usada = v && (numES(v.cant) > 0 || numES(v.precio) > 0);
            if (usada) algunaUsada = true;
            else wsPres.getRow(fila).hidden = true;
          });
          if (!algunaUsada) {
            wsPres.getRow(cap.items_start - 1).hidden = true;
          }
        });
      }
    ...
    <div className="min-h-screen" style={{ background: PAPER, fontFamily: "'IBM Plex Sans', system-ui, sans-serif" }}>
      ...
      <div className="p-4 max-w-xl mx-auto">
        ...
        <label className="flex items-center gap-2 mb-3 text-[12px] cursor-pointer" style={{ color: NAVY }}>
          ...
          Ocultar en el Excel las actividades que no se usaron (quedan ocultas, no se borran — se pueden mostrar de nuevo desde Excel)
        </label>
